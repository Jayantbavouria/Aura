import { db } from "@/db";
import { agents, meetings, user as users } from "@/db/schema";
  ...
  getTranscript: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const transcript = await fetch(existingMeeting.transcriptUrl)
        .then(res => res.text()
          .then(text => JSON.parse(text) as StreamTranscriptItem[])
          .catch(() => { return []; })
        );
      const speakerIds = [
        ...new Set(transcript.map(item => item.speaker_id))
      ];
      const userSpeakers = await db.select().from(users).where(inArray(users.id, speakerIds)).then(users => {
    ...
    .query(async ({ input, ctx }) => {
      const [existingMeeting] = await db
        .select({
          ...
          agent: agents,
          duration: sql<number>`EXTRACT(EPOCH FROM (ended_at - scheduled_at))`.as(
            "duration"
          ),
        })
        .from(meetings)
