...
import { streamVideo } from "@/lib/stream-vid";
...
export async function POST(req: NextRequest) {
  ...
  if (eventType === "call.session_started") {
    ...
    try {
      ...
      if (!realtimeClient) {
        console.error("connectOpenAi returned no client");
        return NextResponse.json({ error: "Failed to create realtime client" }, { status: 500 });
      }
      ...
      console.log("OpenAI realtime client connected for meeting", meetingId);
    } catch (err) {
      console.error("Error initializing OpenAI realtime client:", err);
      return NextResponse.json({ error: "Realtime client init failed" }, { status: 500 });
    }
  }
